...
class APIResponse
{
    ...
    public function getResponseStructure($success = false, $payload = null, $message = '', $debug = null)
    {
        if (isset($debug)) {
            ...
            $data = [
                'success' => $success,
                'message' => $message,
                'payload' => !empty($payload) ? $payload : (object)[]
            ];
        }
     ...
     * @return JsonResponse
     */
    public function respondUnauthorizedWithMessageAndPayload($payload = null, $message = "Ok")
    {
        $this->setStatusCode(ResponseHTTP::HTTP_UNAUTHORIZED);
        $data = $this->getResponseStructure(true, $payload, $message);
     ...
     * @return JsonResponse
     */
    public function respondNotAcceptableWithMessageAndPayload($payload = null, $message = "Ok")
    {
        $this->setStatusCode(ResponseHTTP::HTTP_NOT_ACCEPTABLE);
        $data = $this->getResponseStructure(true, $payload, $message);
    ...
    public function respondWithError($message = "Error", $e = null, $data = null)
    {
        ...
        if (\App::environment('local', 'staging') && isset($e)) {
            $debug_message = !empty($e[0]) ? $e[0] : __('messages.something_went_wrong');
            $data = $this->getResponseStructure(false, $data, $debug_message);
        } else {
     ...
     * @return mixed
     */
    public function respondUnauthorizedWithData($e, $data, $message = "Unauthorized")
    {
        return $this->setStatusCode(ResponseHTTP::HTTP_UNAUTHORIZED)
            ->respondWithError($message, $e, $data);
    }
     ...
     * @return JsonResponse|null
     */
    public function respondWithCustomErrorUsingKey($error, $message, $e = null, $data = null)
    {
        if (\App::environment('local', 'staging') && isset($e)) {
            $debug_message = $e;
            $data = $this->setStatusCode(ResponseHTTP::HTTP_UNPROCESSABLE_ENTITY)->getCustomResponseStructure(
                false,
                $error,
                $message,
                $debug_message
     ...
     * @return type
     */
    public function getCustomResponseStructure($success = false, $error = null, $message = '', $debug = null)
    {
        if (isset($debug)) {
            $data = [
                'success' => $success,
    ...
    public function respondPartialContent($message)
    {
        return $this->setStatusCode(ResponseHTTP::HTTP_PARTIAL_CONTENT)
            ->respondWithMessage($message);
    }

    public function respondAlreadyReportedContent($message)
    {
        return $this->setStatusCode(ResponseHTTP::HTTP_ALREADY_REPORTED)
            ->respondWithMessage($message);
    }
